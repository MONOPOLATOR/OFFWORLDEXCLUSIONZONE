...
public class LevelEditor : MonoBehaviour 
{
    ...
    */
	...
	void Update()
	{
        if (mouseInCamera())
        {
            ...
            if (Input.GetMouseButtonDown(1))
            {
                ...
                //BRUSH
                //row 1
                SetTile(1, x - 1, y + 1, 0, BLANK_ST);
                SetTile(1, x + 0, y + 1, 0, BLANK_ST);
                SetTile(1, x + 1, y + 1, 0, BLANK_ST);
                //row 2
                SetTile(1, x - 1, y + 0, 0, BLANK_ST);
                SetTile(1, x + 0, y + 0, 0, BLANK_ST);
                SetTile(1, x + 1, y + 0, 0, BLANK_ST);
                //row 3
                SetTile(1, x - 1, y - 1, 0, BLANK_ST);
                SetTile(1, x + 0, y - 1, 0, BLANK_ST);
        ...
        for (int l = 0; l < TILE_LAYERS; l++)
        {
            ...
            for (int y = 0; y < LEVEL_SIZE; y++)
            {
                for (int x = 0; x < LEVEL_SIZE; x++)
                {
                    ...
                    if (subb1 != BLANK_ST) { Debug.Log("Loaded a tile at " + l + "," + x + "," + y + " (" + tilepos + ")"); }
    ...
    void SetTile(int layer, int x, int y, int tileSet, byte subTile)
    {
        ...
        {
            Debug.Log("Error: tile " + x + "," + y + " out of bounds.");
        }
    ...
    bool CheckTile(int l, int x, int y)
    {
        if(!ValidateCoords(x, y)) { return false; }
        if(workingTiles[l, y, x].subTile == BLANK_ST) { return false; } else { return true; }
    }
    byte CheckSurroundingEightTiles (int l, int x, int y)
    {
        int ind2Check = 0;
        //this SHOULD work but i havent tested it and its math
        if (CheckTile(l, x + 0, y + 1)) { ind2Check = ind2Check | 001; }
        if (CheckTile(l, x + 1, y + 0)) { ind2Check = ind2Check | 002; }
        if (CheckTile(l, x + 0, y - 1)) { ind2Check = ind2Check | 004; }
        if (CheckTile(l, x - 1, y + 0)) { ind2Check = ind2Check | 008; }

        //if (CheckTile(l, x + 1, y + 1)) { ind2Check = ind2Check | 016; }
        //if (CheckTile(l, x + 1, y - 1)) { ind2Check = ind2Check | 032; }
        //if (CheckTile(l, x - 1, y - 1)) { ind2Check = ind2Check | 064; }
        //if (CheckTile(l, x - 1, y + 1)) { ind2Check = ind2Check | 128; }
        //FOR NOW DONT CHECK SURROUNDING MAPS
        //
        return (byte)ind2Check;
    }
}